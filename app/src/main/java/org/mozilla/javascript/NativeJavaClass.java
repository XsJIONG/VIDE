...
public class NativeJavaClass extends NativeJavaObject implements Function
{
    ...
    {
        Object instance = constructInternal(args, ctor, cx, scope);
    ...
    static Object constructInternal(Object[] args, MemberBox ctor, Context cx, Scriptable scope)
    {
        ...
        Object retval=null;
		if (cx.isVposed()) {
			VposedListener li=cx.getVposed(ctor.ctor());
			if (li == null)
				retval = ctor.newInstance(args);
			else {
				Vposed v=new Vposed();
				li.beforeInvoke(cx, scope, null, args, v);
				if (!v.isRefuseInvokeMethod()) retval = ctor.newInstance(args);
				if (v.isReturnOverrided())
					retval = v.getReturnObject();
				li.afterInvoke(cx, scope, null, args, v);
			}
		} else retval = ctor.newInstance(args);
		return retval;
    }
