...
// The method must NOT be public or protected
	...
	private boolean isVposed=false;
	private HashMap<Method,VposedListener> VposedsM=new HashMap<>();
	private HashMap<Constructor,VposedListener> VposedsC=new HashMap<>();
	public void injertMethod(Method one, VposedListener listener) {
		if (listener == null) return;
		VposedsM.put(one, listener);
	}
	public void cancelInjertMethod(Method one) {
		VposedsM.remove(one);
	}
	public void clearInjertMethod() {
		VposedsM.clear();
	}
	public void injertConstructor(Constructor one, VposedListener listener) {
		if (listener == null) return;
		VposedsC.put(one, listener);
	}
	public void cancelInjertConstructor(Constructor one) {
		VposedsC.remove(one);
	}
	public void clearInjertConstructor() {
		VposedsC.clear();
	}
	public VposedListener getVposed(Method m) {
		return VposedsM.get(m);
	}
	public VposedListener getVposed(Constructor c) {
		return VposedsC.get(c);
	}
	public boolean isVposed() {
		return isVposed;
	}
	public void setVposed(boolean flag) {
		isVposed = flag;
	}
}
