...
public class CardView extends FrameLayout implements CardViewDelegate {
	...
	private float nowElevation=getCardElevation();
	...
	ElevationAnimate ani=null;
	private boolean contain(MotionEvent event) {
		Rect r=new Rect();
		getDrawingRect(r);
		return r.contains((int) event.getX(), (int) event.getY());
	}
	private void setRealElevation(float radius) {
		IMPL.setElevation(this, radius);
	}
	@Override
	...
	private void onEvent(MotionEvent event) {
		switch (event.getAction()) {
			case MotionEvent.ACTION_DOWN:
				if (ani != null && ani.isRunning()) ani.stop();
				ani = new ElevationAnimate(this, IMPL.getElevation(this), translationZ);
				ani.start();
				break;
			case MotionEvent.ACTION_MOVE:
				if (!contain(event)) {
					if (ani != null && ani.isRunning()) ani.stop();
					ani = new ElevationAnimate(this, IMPL.getElevation(this), nowElevation);
					ani.start();
				}
				break;
	...
	private static class ElevationAnimate extends VAnimation<CardView> {
		private float from=-1, to=-1;
		public ElevationAnimate(CardView v, float from, float to) {
			super(v);
			this.from = from;
			this.to = to;
