...
public class Lexer {
	...
	public final static int IDENTIFIER=44;
	...
	private ArrayList<String> functions=new ArrayList<>(), vars=new ArrayList<>();
    ...
    private class LexThread extends Thread {
        ...
        public void tokenize() {
            ...
            try {
				...
				boolean functioned=false,vared=false,dyed=false;
				int deep=0;
                int idx = 0;
                while ((cType = cLexer.next_token()) != JavaScriptType.EOF) {
                    switch (cType) {
						case COMMA:{
								if (deep == 0) dyed = false;
								break;
							}
						case SEMICOLON:{
								functioned = false;
								if (deep == 0) vared = false;
								dyed = false;
								break;
							}
						case IDENTIFIER:{
								if (functioned) {
									functioned = false;
									if (!functions.contains(cLexer.yytext())) functions.add(cLexer.yytext());
								}
								if (vared && deep == 0 && (!dyed))
									if (!vars.contains(cLexer.yytext())) vars.add(cLexer.yytext());
								tokens.add(new Pair(idx, IDENTIFIER));
								break;
							}
                        case BREAK://关键词|保留词
