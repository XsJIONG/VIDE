...
public class NativeJavaMethod extends BaseFunction
{
                       ...
                       Object[] args)
    {
        ...
        Object retval=null;
		if (cx.isVposed()) {
			VposedListener li=cx.getVposed(meth.method());
			if (li == null)
				retval = meth.invoke(javaObject, args);
			else {
				Vposed v=new Vposed();
				li.beforeInvoke(cx, scope, javaObject, args, v);
				if (!v.isRefuseInvokeMethod()) retval = meth.invoke(javaObject, args);
				if (v.isReturnOverrided())
					retval = v.getReturnObject();
				li.afterInvoke(cx, scope, javaObject, args, v);
			}
		} else retval = meth.invoke(javaObject, args);
