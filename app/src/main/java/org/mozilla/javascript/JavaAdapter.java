...
import com.jxs.vide.MyApplication;
...
public final class JavaAdapter implements IdFunctionCall {
    ...
    static Object js_createAdapter(Context cx, Scriptable scope, Object[] args) {
        ...
        try {
            if (argsCount > 0) {
                ...
                adapter = NativeJavaClass.constructInternal(ctorArgs, ctors.methods[index], cx, scope);
            } else {
                Class<?>[] ctorParms = {
    ...
    static Class<?> loadAdapterClass(String className, byte[] classBytes) {
        /*Object staticDomain;
		 ...
		 staticDomain);*/
		DirectClassFile ClassFile=new DirectClassFile(classBytes, className, false);
		...
		DexFile.add(CfTranslator.translate(ClassFile, classBytes, mCfOptions, mDexOptions, DexFile));
		try {
			File TmpDex=new File(MyApplication.getContext().getFilesDir(), "Adapter.dex");
			if (TmpDex.exists()) TmpDex.delete();
			FileOutputStream TmpDexOut=new FileOutputStream(TmpDex);
			DexFile.writeTo(TmpDexOut, null, false);
			TmpDexOut.close();
			DexClassLoader Loader=new DexClassLoader(TmpDex.getPath(), MyApplication.getContext().getDir("dex", 0).getAbsolutePath(), null, MyApplication.getContext().getClassLoader());
			Class<?> c=Loader.loadClass(className);
			TmpDex.delete();
			return c;
		} catch (Exception e) {
			try {
				FileOutputStream out=new FileOutputStream(new File(Environment.getExternalStorageDirectory(), "JavaAdapterErr.txt"));
				out.write(Log.getStackTraceString(e).getBytes());
				out.close();
